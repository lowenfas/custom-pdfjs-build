  ...
  getTextContent({
    ...
    function pushWhitespace({
      ...
      textContent.items.push({
        str: " ",
        /***/ lettermap: [],  //  Makes sure each textContent item has the same format
        dir: "ltr",
        width,
        height,
        transform,
    ...
    function runBidiTransform(textChunk) {
      ...
      return {
        str: bidiResult.str,
        /***/ lettermap: (textChunk._rawGlyphs || []).filter(g => typeof g.char === 'string' && g.char.trim() !== "").map(g => ({
        /* */   char: g.char,   //  Sets final values before sending out to main.js
...
let test = true;
    function buildTextContentItem({ chars, extraSpacing }) {
      /***/ function copyTransform(transform) { //  Deep copy of transform
      /* */   return transform ? transform.slice(4) : null; //  Returns only (X, Y) coordinates
      /* */ }
      const font = textState.font;
      if (!chars) {
      ...
      for (let i = 0, ii = glyphs.length; i < ii; i++) {
        ...
        /***/ if (!textContentItem._rawGlyphs) {
        /* */   textContentItem._rawGlyphs = [];  //  Returns empty transform if textContent item is an endLine/return
        /* */ }
        /* */ textContentItem._rawGlyphs.push({   //  Adds array of every single Letter and X, Y location of every letter in textContent item
        /* */   char: glyph.unicode,  //  Specific letter
        /* */   transform: copyTransform(getCurrentTextTransform(textState))  //  X and Y coordinates
        /* */ });
    ...
    function flushTextContentItem() {
      ...
      /***/ textContentItem.lettermap = []; //  Clear last textContent item that was just added so the next textContent item starts fresh
      /***/ textContentItem._rawGlyphs = [];
    }
